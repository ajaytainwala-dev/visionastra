...
import { AssignmentCreateSchema, AssignmentUpdateSchema } from '@/lib/validation/course-schemas'
...
export async function GET(req: Request) {
  ...
  const allowed = await callerHas(user.id, 'assignments', 'read')
...
export async function POST(req: Request) {
  ...
  const allowed = await callerHas(user.id, 'assignments', 'create')
  if (!allowed) return NextResponse.json({ error: 'forbidden' }, { status: 403 })

  try {
    const body = await req.json()
    const parsed = AssignmentCreateSchema.parse(body)
    const created = await courseService.createAssignment(user.id, parsed)
    return NextResponse.json({ data: created })
  } catch (e: any) {
    if (e instanceof z.ZodError) {
      await writeAudit({ userId: user.id, action: 'assignments.create', resource_type: 'assignments', status: 'error', details: { validation_error: true } })
...
export async function PUT(req: Request) {
  ...
  const allowed = await callerHas(user.id, 'assignments', 'update')
  if (!allowed) return NextResponse.json({ error: 'forbidden' }, { status: 403 })
  ...
  try {
    ...
    const updated = await courseService.updateAssignment(user.id, parsed)
...
export async function DELETE(req: Request) {
  ...
  const allowed = await callerHas(user.id, 'assignments', 'delete')
  if (!allowed) return NextResponse.json({ error: 'forbidden' }, { status: 403 })
  ...
  try {
    ...
    await courseService.deleteAssignment(user.id, id)
